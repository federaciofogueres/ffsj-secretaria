...
import { ModificacionConfirmacionResultado, ModificacionPreparacion, RubiApiService } from './rubi-api.service';
...
export class RubiModificacionComponent implements OnInit, OnDestroy {
  ...
  prepare(): void {
    ...
    if (!Object.keys(cambios).length) { this.errorKey = 'rubi.mod.error.noChanges'; return; }
    this.loading = true;
  ...
  private invalidate(): void { this.discardPrepared(); this.prepared = null; this.clearTimer(); this.errorKey = 'rubi.mod.error.contextChanged'; this.preparationStateChanged.emit(false); }
  private clearSensitive(): void { this.form.reset(); this.selected = null; this.selectedCargoIds.clear(); this.prepared = null; this.confirmationAccepted = false; this.clearTimer(); this.preparationStateChanged.emit(false); }
