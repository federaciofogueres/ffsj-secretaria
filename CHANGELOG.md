...
## 0.43.5#ESMERALDA — Hotfix crítico de Incidencias, Inscripciones y búsqueda flexible en Solicitudes

> Cuatro regresiones detectadas tras `0.43.x`, dos críticas: Asociación no podía continuar una conversación de Incidencias sin que Administración pulsara "Devolver a asociación", y persistía una mezcla real de adjuntos (causa distinta de la ya corregida en `0.43.2#ESMERALDA`).

- **CRÍTICO — condición de carrera real en adjuntos de Incidencias**: `IncidenciasPanelComponent.crear()`/`responder()`/`comentar()` leían `this.selectedFiles`/`this.responseFiles[id]`/`this.commentFiles[id]` **dentro** del `switchMap`, es decir, tras la respuesta del servidor, no en el instante de enviar. Si mientras la petición seguía en vuelo el usuario cambiaba de recurso (dispara el reset de `0.43.2#ESMERALDA`) y empezaba un borrador nuevo, la respuesta tardía podía subir ese borrador nuevo contra el evento equivocado y pisar lo que el usuario seguía escribiendo. Corregido capturando mensaje/adjuntos/`scope`/`scopeId` en el momento exacto de enviar; el composer solo se limpia y la lista solo se recarga si el recurso sigue siendo el mismo cuando llega la respuesta. Mismo patrón corregido en `onAdjuntosEntradaChange()` de `InscripcionesComponent` (una subida de documento lenta ya no sobrescribe `adjuntosEntrada` si mientras tanto se abrió otra entrada). El bug de `0.43.2#ESMERALDA` (reset en `ngOnChanges`) sigue intacto y sin relación con este; los tests que antes daban falsa confianza usaban observables síncronos (`of(...)`) que no podían reproducir ninguna ventana asíncrona real — los tests nuevos usan `Subject` para forzarla.
- **Respuesta de Asociación en Incidencias**: el frontend ya mostraba correctamente el composer con la incidencia en `respondida` (fix de `0.43.3#ESMERALDA`), pero el backend (`ffsj-secretaria-api`) seguía rechazando el envío real con 404 salvo que Administración hubiera devuelto la incidencia a `abierta` primero. Corregido en la API; sin cambios de frontend necesarios en este punto.
- **Apertura del detalle de Inscripciones para Asociación**: tras enviar una inscripción nueva se abría una sección plana antigua (`associationMode = 'summary'`) en vez del dialog compartido con Administración de `0.43.0#ESMERALDA`. Como una entrada recién creada nunca tiene incidencias todavía, se percibía como "sin incidencias → vista antigua". Se elimina por completo el modo `'summary'` (tipo `AssociationMode`, rama muerta en `associationStep` y sección `summary-panel`/`cerrarResumen()` en la plantilla, ya sin ningún llamador): ahora siempre se abre el mismo dialog, tenga o no incidencias.
- **Búsqueda flexible en Solicitudes**: el campo "Buscar por: Asociación" exigía seleccionar una opción exacta del `<datalist>`; texto parcial sin selección exacta no aplicaba ningún filtro. Ahora resuelve el texto parcial (insensible a mayúsculas) contra la lista de asociaciones ya cargada y envía todos los ids candidatos (`asociacionIds`) a la API; si el texto no coincide con ninguna asociación real, se envía un id imposible (`-1`) para mostrar explícitamente 0 resultados en vez de ignorar el filtro y mostrar todas las solicitudes. La búsqueda por Nº de solicitud ya soportaba coincidencia parcial (`LIKE '%valor%'`); el 500 real que producía (causa raíz: bug de collation en el backend, ver su changelog) queda resuelto ahí.
- Sin regresiones: filtros compactos, ordenación por columnas, paginación y toast de error de Solicitudes (`0.43.4#ESMERALDA`); reglas de cierre/devolución/acciones administrativas de Incidencias.
- Validación técnica: `ng build --configuration=development` correcto y suite completa en verde (**393/393**: 386 previos + 7 nuevos). Backend: 812/812 y `openapi:check` correcto.
- Validación manual pendiente: confirmación de Fran en navegador de (1) que Asociación puede mantener una conversación de varios turnos sin ninguna acción de Administración de por medio, (2) que el dialog nuevo de Inscripciones se abre siempre tras enviar, y (3) de la búsqueda parcial por Nº de solicitud y por Asociación en el patrón compacto real.

## 0.43.4#ESMERALDA — Corrección del buscador de Solicitudes y refactor de filtros

> Reporte: al buscar en `Solicitudes` el endpoint devolvía 500. Auditoría extremo a extremo en `ffsj-secretaria-api` (análisis estático, pruebas contra `db.query` mockeado con decenas de combinaciones, y verificación en vivo contra una base de datos real y un servidor HTTP real con JWT válido) no logró reproducir ningún 500 con el código y esquema actuales. Se confirmaron en su lugar gaps funcionales reales: el placeholder del buscador prometía filtrar por "tipo o estado" sin hacerlo, no existía forma de buscar por asociación ni por fecha de alta, y el orden vivía en un selector independiente en vez de las cabeceras de la tabla. El rediseño elimina por construcción la ambigüedad que producía resultados inesperados.

- **Backend** (`ffsj-secretaria-api`, `0.43.4#ESMERALDA`, ya cerrada y mergeada a `develop`): `busqueda` pasa a filtrar exclusivamente por Nº de solicitud (antes adivinaba también por `asociacion_id`); se añaden los filtros reales `asociacionId` (reutilizando la condición ya existente para la vista de asociación) y `fechaAlta` (fecha exacta); el antiguo parámetro único `orden` se sustituye por `ordenCampo`/`ordenDireccion`, con las 7 columnas ordenables (`numero`, `asociacion`, `registros`, `tipo`, `estado`, `fecha_alta`, `fecha_entrada`) mapeadas de forma segura a columnas reales — un `ordenCampo` desconocido nunca genera SQL inválido, cae a `fecha_alta`. 16 tests nuevos fijan el contrato.
- **Frontend — nuevo patrón compacto de búsqueda/filtros**: sustituida la zona de filtros (buscador de texto + selectores de Tipo/Estado/Orden + botón Buscar) por un patrón "buscar por campo": selector `Buscar por` (Nº solicitud / Asociación / Tipo / Estado / Fecha de alta) que adapta el control de valor correspondiente (texto, `<datalist>` de asociaciones ya cargadas, o los selectores existentes), un único filtro activo se muestra como chip cerrable, y `Limpiar filtros` restablece los valores por defecto (incluye el filtro de "Solo problemáticas", que se aplica de forma independiente e inmediata). Se elimina el selector de orden independiente.
- **Ordenación por columnas**: los 7 encabezados de columna correspondientes a los campos ordenables del backend son ahora botones (`ordenarPor(campo)`) con indicador visual (`bi-chevron-up`/`down`/`expand`) y `aria-sort` correcto; pulsar la misma columna invierte la dirección, pulsar otra columna la selecciona con dirección ascendente. La ordenación se integra con los filtros y la paginación existentes sin romper ninguno (misma llamada a `getSolicitudesGlobal`).
- **Toast de error**: el bloque de error embebido en la página (`<div class="alert alert-danger">`) se sustituye por un `ToastComponent` nuevo y reutilizable (esquina inferior derecha, cerrable con la X o con click en cualquier parte del toast, `role="alert"`/`aria-live="assertive"`). El aviso de éxito tras validar/rechazar una solicitud queda fuera de alcance de esta versión y no se ha tocado.
- Se mantiene sin cambios: contadores, paginación, listado, botón Detalles y las reglas de negocio del módulo (filtro por defecto `Estado: Enviada` al cargar, comportamiento de "Solo problemáticas", etc.).
- Validación técnica: `ng build --configuration=development` correcto y suite completa en verde (**386/386**). Backend: 804/804 y `openapi:check` correcto (cierre ya registrado en el `CHANGELOG.md` de `ffsj-secretaria-api`).
- Validación en vivo explícita contra el endpoint ya mergeado: 18 combinaciones de búsqueda/tipo/estado/asociación/fecha de alta/las 7 columnas de orden (ambas direcciones)/paginación/filtros combinados, todas con respuesta `200` — sin ningún `500`.
- Validación manual pendiente: confirmación de Fran en navegador del aspecto visual del nuevo patrón compacto frente a los mockups aprobados, y de la experiencia de uso completa (buscar → filtrar → ordenar → paginar → Detalles).
